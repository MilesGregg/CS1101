...
; String String Natural Natural Boolean -> restaurant
; consumes a name, food, tables, seats, and vegetarian? to produce a restaurant
; A restaurant consists of: A name(the name of the restaurant)
;                           food(type of food the restaurant sells)
;                           tables(how many tables the restaurant has available)
;                           seats(how many seats the restaurant has available)
;                           vegetarian?(if the restaurant is vegtarian or not)
(define-struct restaurant (name food tables seats vegetarian?))

(define R1 (make-restaurant "Le Pain" "French" 10 40 true))
(define R2 (make-restaurant "Anthony" "Pizza" 5 20 false))
(define R3 (make-restaurant "Longhorn" "Steakhouse" 50 250 true))
...
; consumes a month, day, and year to produce a date
; A date consists of: A month(the month of th year)
;                     day(which day of the month)
;                     year(which year it is)
(define-struct date (month day year))


; String String String Natural Natural -> reservation
; consumes a restaurant-name, person, phone, date, and party to produce a reservation
; A restaurant consists of: A restaurant-name(the name of the restaurant)
;                           person(the name of the person making the reservation)
;                           phone(their phone number)
;                           datewhat date the reservation is for)
;                           party(how many people are in their party)
(define-struct reservation (restaurant-name person phone date party))

(define Res (make-reservation "Le Pain" "Bob" "111-212-3333" (make-date 9 6 2020) 12))
(define Res1 (make-reservation "Anthony" "Joe" "123-212-3333" (make-date 9 4 2020) 10))
(define Res2 (make-reservation "Longhorn" "Lily" "511-212-3233" (make-date 9 9 2020) 4))
...
;; Test Cases:
(check-expect (add-to-party Res 12) (make-reservation "Le Pain" "Bob" "111-212-3333" (make-date 9 6 2020) 24))
(check-expect (add-to-party Res1 -6) (make-reservation "Anthony" "Joe" "123-212-3333" (make-date 9 4 2020) 4))
(check-expect (add-to-party Res2 0) (make-reservation "Longhorn" "Lily" "511-212-3233" (make-date 9 9 2020) 4))
